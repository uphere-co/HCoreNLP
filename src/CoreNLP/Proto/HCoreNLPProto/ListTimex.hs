...
module CoreNLP.Proto.HCoreNLPProto.ListTimex (ListTimex(..)) where
import Prelude ((+), (/))
...
data ListTimex = ListTimex{timexes :: !(P'.Seq CoreNLPProtos.Timex)}
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             10 -> Prelude'.fmap (\ !new'Field -> old'Self{timexes = P'.append (timexes old'Self) new'Field}) (P'.wireGet 11)
...
instance P'.TextMsg ListTimex where
  textPut msg
   = do
       P'.tellT "timexes" (timexes msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'timexes]) P'.spaces
    ...
    where
        parse'timexes
         = P'.try
            (do
               v <- P'.getT "timexes"
               Prelude'.return (\ o -> o{timexes = P'.append (timexes o) v}))