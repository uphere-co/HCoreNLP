...
module CoreNLP.Proto.CoreNLPProtos.IndexedWord (IndexedWord(..)) where
import Prelude ((+), (/))
...
data IndexedWord = IndexedWord{sentenceNum :: !(P'.Maybe P'.Word32), tokenIndex :: !(P'.Maybe P'.Word32),
                               docID :: !(P'.Maybe P'.Word32), copyCount :: !(P'.Maybe P'.Word32)}
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             8 -> Prelude'.fmap (\ !new'Field -> old'Self{sentenceNum = Prelude'.Just new'Field}) (P'.wireGet 13)
             16 -> Prelude'.fmap (\ !new'Field -> old'Self{tokenIndex = Prelude'.Just new'Field}) (P'.wireGet 13)
             24 -> Prelude'.fmap (\ !new'Field -> old'Self{docID = Prelude'.Just new'Field}) (P'.wireGet 13)
             32 -> Prelude'.fmap (\ !new'Field -> old'Self{copyCount = Prelude'.Just new'Field}) (P'.wireGet 13)
...
instance P'.TextMsg IndexedWord where
  textPut msg
   = do
       P'.tellT "sentenceNum" (sentenceNum msg)
       P'.tellT "tokenIndex" (tokenIndex msg)
       P'.tellT "docID" (docID msg)
       P'.tellT "copyCount" (copyCount msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'sentenceNum, parse'tokenIndex, parse'docID, parse'copyCount]) P'.spaces
    ...
    where
        parse'sentenceNum
         = P'.try
            (do
               ...
               Prelude'.return (\ o -> o{sentenceNum = v}))
        parse'tokenIndex
         = P'.try
            (do
               v <- P'.getT "tokenIndex"
               Prelude'.return (\ o -> o{tokenIndex = v}))
        parse'docID
         = P'.try
            (do
               v <- P'.getT "docID"
        ...
        parse'copyCount
         = P'.try
            (do
               v <- P'.getT "copyCount"
               Prelude'.return (\ o -> o{copyCount = v}))