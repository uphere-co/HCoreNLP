...
module CoreNLP.Proto.CoreNLPProtos.Sentiment (Sentiment(..)) where
import Prelude ((+), (/), (.))
...
instance Prelude'.Enum Sentiment where
  ...
  toEnum
   = P'.fromMaybe (Prelude'.error "hprotoc generated code: toEnum failure for type CoreNLP.Proto.CoreNLPProtos.Sentiment") .
      toMaybe'Enum
  succ STRONG_NEGATIVE = WEAK_NEGATIVE
  succ WEAK_NEGATIVE = NEUTRAL
  succ NEUTRAL = WEAK_POSITIVE
  succ WEAK_POSITIVE = STRONG_POSITIVE
  succ _ = Prelude'.error "hprotoc generated code: succ failure for type CoreNLP.Proto.CoreNLPProtos.Sentiment"
  pred WEAK_NEGATIVE = STRONG_NEGATIVE
  pred NEUTRAL = WEAK_NEGATIVE
  pred WEAK_POSITIVE = NEUTRAL
  pred STRONG_POSITIVE = WEAK_POSITIVE
  pred _ = Prelude'.error "hprotoc generated code: pred failure for type CoreNLP.Proto.CoreNLPProtos.Sentiment"
...
instance P'.ReflectEnum Sentiment where
  ...
  reflectEnumInfo _
   = P'.EnumInfo (P'.makePNF (P'.pack ".edu.stanford.nlp.pipeline.Sentiment") ["CoreNLP", "Proto"] ["CoreNLPProtos"] "Sentiment")
      ["CoreNLP", "Proto", "CoreNLPProtos", "Sentiment.hs"]
