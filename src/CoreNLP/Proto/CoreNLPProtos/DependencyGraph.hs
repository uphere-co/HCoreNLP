...
module CoreNLP.Proto.CoreNLPProtos.DependencyGraph (DependencyGraph(..)) where
import Prelude ((+), (/))
...
data DependencyGraph = DependencyGraph{node :: !(P'.Seq CoreNLPProtos.DependencyGraph.Node),
                                       edge :: !(P'.Seq CoreNLPProtos.DependencyGraph.Edge), root :: !(P'.Seq P'.Word32)}
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             ...
             18 -> Prelude'.fmap (\ !new'Field -> old'Self{edge = P'.append (edge old'Self) new'Field}) (P'.wireGet 11)
             24 -> Prelude'.fmap (\ !new'Field -> old'Self{root = P'.append (root old'Self) new'Field}) (P'.wireGet 13)
             26 -> Prelude'.fmap (\ !new'Field -> old'Self{root = P'.mergeAppend (root old'Self) new'Field}) (P'.wireGetPacked 13)
...
instance P'.TextMsg DependencyGraph where
  textPut msg
   = do
       ...
       P'.tellT "edge" (edge msg)
       P'.tellT "root" (root msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'node, parse'edge, parse'root]) P'.spaces
    ...
    where
        ...
        parse'edge
         = P'.try
            (do
               v <- P'.getT "edge"
               Prelude'.return (\ o -> o{edge = P'.append (edge o) v}))
        parse'root
         = P'.try
            (do
               v <- P'.getT "root"
               Prelude'.return (\ o -> o{root = P'.append (root o) v}))