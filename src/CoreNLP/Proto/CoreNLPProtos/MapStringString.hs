...
module CoreNLP.Proto.CoreNLPProtos.MapStringString (MapStringString(..)) where
import Prelude ((+), (/))
...
data MapStringString = MapStringString{key :: !(P'.Seq P'.Utf8), value :: !(P'.Seq P'.Utf8)}
   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             10 -> Prelude'.fmap (\ !new'Field -> old'Self{key = P'.append (key old'Self) new'Field}) (P'.wireGet 9)
